...
The quantity of main interest is the cross section $\sigma$, where the groundstate $(|i\rangle, E_i)$ appears, as well as the intermediate absorption state $(|m\rangle\!\!=\!\!T_a^\mathbf{q}|i\rangle, E_m)$ and the final emission state $(|f\rangle\!\!=\!\!T_e^\mathbf{q}|m\rangle, E_f)$. $\Gamma$ is the quasiparticle lifetime.
\end{beamercolorbox}
\end{block}
\end{column}
...
\setbeamercolor{coloredboxstuff}{bg=ta2orange!0}
\begin{beamercolorbox}[wd=\textwidth]{coloredboxstuff}
The model of interest is the AAIM, corresponding to an impurity with \textcolor[rgb]{0.25,0.46,0.02}{core} and \textcolor[rgb]{0,0.32,0.69}{valence} states, the latter hybridizing with a \textcolor[rgb]{0.75,0.47,0}{non-interacting bath}.
...
We do a one band model benchmark with copper oxide CuO
